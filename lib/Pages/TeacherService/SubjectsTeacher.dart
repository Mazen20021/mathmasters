...
class TeacherSubjects extends StatefulWidget {
  ...
  @override
  State<TeacherSubjects> createState() {
    return _subjects(role: role, subjectName: subjectName);
  }
}

class _subjects extends State<TeacherSubjects> {
  final String subjectName;
  final String role;
  ...
  Future<void> _update(String newUrl, int index, String name) async {
    ...
    final docRef = firestore.collection('Subjects').doc(subjectName);
    ...
    try {
      ...
      if (docSnapshot.exists) {
        // Retrieve the existing list for the specified name
        List<dynamic> Books = docSnapshot.data()?[name] ?? [];
        ...
        await docRef.update({name: Books}).catchError((e) async {
          await docRef.set({name: Books});
        });
      } else {
        // Document does not exist, create a new document with the specified name and data
        await docRef.set({name: List.filled(index + 1, '')..[index] = newUrl});
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Future<void> _updateFirestore(
    ...
    try {
      ...
      if (docSnapshot.exists) {
        // Retrieve the existing 'Videos' list or initialize it as an empty list if it doesn't exist
        List<dynamic> videos = docSnapshot.data()?['Videos'] ?? [];

        // Check if the index is within range or needs to be extended
        if (index < videos.length) {
          // Update the specific index with the new data
          Map<String, dynamic> updatedVideo = videos[index];
        ...
        // Update the Firestore document with the new 'Videos' list
        await docRef.update({'Videos': videos}).catchError((e) async {
          await docRef.set({'Videos': videos});
        });
      } else {
        // If the document does not exist, create a new one with the 'Videos' list
        await docRef.set({
          'Videos': List.generate(
              index + 1,
              (i) => i == index
                  ? {'Code': newCode, 'Video': newUrl}
                  : {'Code': '', 'Video': ''})
        });
      }
    } catch (e) {
  ...
  Future<void> _insertData(String name) async {
    ...
    try {
      ...
      if (docSnapshot.exists) {
        // Assuming the document contains a list field named 'Name'
        List<dynamic>? namesList = docSnapshot.get('Name') as List<dynamic>?;
        ...
        if (namesList != null) {
          ...
          await firestore
              ...
              .update({'Name': namesList});

          // Update the local data list to reflect the change in the UI
          setState(() {
            data.clear();
            for (var name in namesList) {
              data.add(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('New name inserted successfully.'),
            ),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('No data found for the specified field.'),
            ),
          );
        }
      } else {
